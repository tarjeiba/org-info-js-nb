...
switch(a){case"TOC":case"TOC_DEPTH":case"MOUSE_HINT":case"HJELP":case"VIEW":case"HIDE_TOC":case"LOCAL_TOC":case"OCCUR":d.set(a,decodeURIComponent(b));
...
m.eRC();}if(m.STARTUP_MESSAGE){m.warn("Denne siden bruker org-info.js. Trykk `?' for mer informasjon.",true);
...
var e=0;for(var d=0;d<this.S.length;++d){this.S[d].J=d;var c='<table class="org-info-js_info-navigation" width="100%" border="0" style="border-bottom:1px solid black;"><tr><td colspan="3" style="text-align:left;border-style:none;vertical-align:bottom;"><span style="float:left;display:inline;text-align:left;">Topp: <a accesskey="t" href="javascript:org_html_manager.go(0);">'+f+'</a></span><span style="float:right;display:inline;text-align:right;font-size:70%;">'+this.LINKS+'<a accesskey="m" href="javascript:org_html_manager.toggleView('+d+');">endre visning</a></span></td></tr><tr><td style="text-align:left;border-style:none;vertical-align:bottom;width:22%">';
if(d>0){c+='<a accesskey="p" href="'+this.S[d-1].L+'" title="Gå til: '+this.rT(this.S[d-1].H.innerHTML)+'">Forrige</a> | ';
}else{c+="Forrige | ";}if(d<this.S.length-1){c+='<a accesskey="n" href="'+this.S[d+1].L+'" title="Gå til: '+this.rT(this.S[d+1].H.innerHTML)+'">Neste</a>';
}else{c+="Neste";}c+='</td><td style="text-align:center;vertical-align:bottom;border-style:none;width:56%;">';
if(d>0&&this.S[d].P.P){c+='<a href="'+this.S[d].P.L+'" title="Gå til: '+this.rT(this.S[d].P.H.innerHTML)+'"><span>'+this.S[d].P.H.innerHTML+"</span></a>";
}else{c+="<span>"+this.S[d].H.innerHTML+"</span>";}c+='</td><td style="text-align:right;vertical-align:bottom;border-style:none;width:22%">';
c+=(d+1)+"</td></tr></table>";this.S[d].BS=document.createElement("div");this.S[d].BS.innerHTML='<div class="org-info-js_header-navigation" style="display:inline;float:right;text-align:right;font-size:70%;font-weight:normal;">'+this.LINKS+'<a accesskey="m" href="javascript:org_html_manager.toggleView('+d+');">endre visning</a></div>';
...
a.className="org-info-js_local-toc";c="Innhold:<br /><ul>";for(var b=0;b<this.S[d].CH.length;
...
var slide=this.SLIDE_VIEW;eval("this."+eval_key+"="+eval_val+";");}else{if("HJELP"==eval_key){eval("this.STARTUP_MESSAGE="+eval_val+";");
...
OrgNode.hE(c.W);if(c.IT){OrgNode.hE(c.IT);}document.getElementsByTagName("h1")[0].style.display="none";
if(c.W.firstChild){c.W.removeChild(c.W.firstChild);}c.R.sAC();for(var b=0;b<c.R.CH.length;
++b){c.R.CH[b].ST=OrgNode.SU;c.R.CH[b].fold();}if(!a){c.ss(d);}if(c.P){OrgNode.sE(c.P);
...
}OrgNode.hE(b.T);OrgNode.sE(b.W);b.R.hAC();OrgNode.hE(b.TOC.D);if(b.P){OrgNode.hE(b.P);
...
var b=a.N.J+1;if(b<a.S.length){a.go(b);}else{a.warn("Allerede siste seksjon.");}},previousSection:function(){var b=this;
var a=b.N.J;if(a>0){b.go(a-1);}else{b.warn("Allerede første seksjon.");}},go:function(b){var a=this;
...
}else{a.HFO=1;a.warn("Historie: Ingen vei framover herfra. Hvilken som helst knapp og `B' for å gå til neste fil.");
...
}else{a.HBO=1;a.warn("Historie: Ingen vei bakover herfra. Hvilken som helst knapp og `b' for å gå til forrige fil.");
}}}},warn:function(c,d,b){var a=this;if(null==b){b="";}a.CI.value=b;if(!d){a.CL.style.color="red";
}a.CL.innerHTML="<span style='float:left;'>"+c+"</span><span style='float:right;color:#aaaaaa;font-weight:normal;'>(hvilken som helst knapp for å fortsette)</span>";
a.sC();window.setTimeout(function(){org_html_manager.CI.value=b;},50);},sR:function(e,b,d,a){var c=this;
if(null==d){d="";}if(null==a){a="";}c.RC=e;c.Rg=true;c.CL.innerHTML="<span style='float:left;'>"+b+"</span><span style='float:right;color:#aaaaaa;font-weight:normal;'>("+a+"ENTER for å lukke)</span>";
...
return;}if(b.SYNTAXING){b.showSyntax();return;}if(b.Mg&&!b.Rg){b.rW();}return;}if(b.Mg&&!b.Rg){b.rW();
return;}else{if(b.Hg){b.showHelp();b.CI.value="";return;}else{if(b.Rg){return;}}}b.CI.value="";
b.CI.blur();if(b.HIDE_TOC&&b.TOC==b.N&&"v"!=c&&"V"!=c&&"\t"!=c){c="b";}else{if("\t"==c){return true;
}else{c=b.trim(c);}}if(1==c.length){if("b"==c){b.poH();}else{if("B"==c){b.poH(true);
...
}else{b.warn("Allerede siste seksjon.");return;}}}else{if("N"==c){if(b.N.J<b.S.length-1){var e=b.N.DEPTH;
var a=b.N.J+1;while(a<b.S.length-1&&b.S[a].DEPTH>=e){if(b.S[a].DEPTH==e){b.go(a);
return;}++a;}}b.warn("Ingen neste søsken.");return;}else{if("p"==c){if(b.N.J>0){b.go(b.N.J-1);
}else{b.warn("Allerede første seksjon.");return;}}else{if("P"==c){if(b.N.J>0){var e=b.N.DEPTH;
var a=b.N.J-1;while(a>=0&&b.S[a].DEPTH>=e){if(b.S[a].DEPTH==e){b.go(a);return;}--a;
}}b.warn("Ingen forrige søsken");}else{if("Q"==c){if(window.confirm("Vil du virkelig stenge filan?")){window.close();
...
b.tG();b.tG();window.print();}else{if("q"==c){window.open(b.BU_QUESTIONS+"?title="+b.rT(b.N.H.innerHTML)+"&body=lenke&labels=spørsmål","_blank");
}else{if("f"==c){if(b.VIEW!=b.INFO_VIEW){b.N.fold();b.N.D.scrollIntoView(true);}}else{if("F"==c){if(b.VIEW!=b.INFO_VIEW){b.tG();
b.N.D.scrollIntoView(true);}}else{if("?"==c||"¿"==c){b.showHelp();}else{if("y"==c){window.open("https://tarjeiba.github.io/promo/syntaks.html","_blank");
}else{if("C"==c){if(b.ST.length){b.showTagsIndex();}else{b.warn("Ingen tags funnet.");
...
}else{b.sR(c,"HTML-link:",'<a href="'+b.BU+b.dT()+'">'+document.title+", Sec. '"+b.rT(b.N.H.innerHTML)+"'</a>","C-c for å kopiere, ");
...
}else{b.sR(c,"Org-link:","[["+b.BU+b.dT()+"]["+document.title+", Sec. '"+b.rT(b.N.H.innerHTML)+"']]","C-c for å kopiere, ");
...
}else{b.sR(c,"Plain URL Link:",b.BU+b.dT(),"C-c for å kopiere, ");window.setTimeout(function(){org_html_manager.CI.select();
},100);}return;}else{if("g"==c){b.sR(c,"Skriv inn kapittelnummer:");return;}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}else{if("s"==c){if(""!=b.OCCUR){b.sR(c,"Søk framover:",b.OCCUR,"ENTER for å bruke forrige, DEL ");
}else{b.sR(c,"Søk framover:",b.OCCUR);}window.setTimeout(function(){org_html_manager.CI.value=org_html_manager.OCCUR;
org_html_manager.CI.select();},100);return;}else{if("S"==c){if(""==b.OCCUR){c="s";
b.sR(c,"Søk framover:");}else{b.RC=c;b.eRC();}return;}else{if("r"==c){if(""!=b.OCCUR){b.sR(c,"Søk bakover:",b.OCCUR,"RET for å bruke forrige, DEL ");
}else{b.sR(c,"Søk bakover:",b.OCCUR);}window.setTimeout(function(){org_html_manager.CI.value=org_html_manager.OCCUR;
org_html_manager.CI.select();},100);return;}else{if("R"==c){if(""==b.OCCUR){c="r";
b.sR(c,"Search backwards:");}else{b.RC=c;b.eRC();}return;}}}}}return;},eRC:function(){var j=this;
var a=j.RC;var k=j.trim(j.CI.value);j.eR();if(""==a||""==k){j.hC();return;}if(a=="g"){var f=j.SN_MAP[k];
if(null!=f){j.hC();j.go(f.J);return;}j.warn("Gå til seksjon: ingen slik seksjon funnet.",false,k);
...
j.hC();j.go(j.S[d].J);return;}}j.warn("Søk framover: tekst ikke funnet.",false,j.OCCUR);
j.OCCUR=g;return;}else{if(a=="S"){for(var d=j.N.J+1;d<j.S.length;++d){if(j.sIO(d)){j.hC();
j.go(j.S[d].J);return;}}j.warn("Søk framover: text ikke funnet.",false,j.OCCUR);return;
}else{if(a=="r"){if(""==k){return false;}if(j.SHO){j.rSH();}var g=j.OCCUR;j.OCCUR=k;
var e=0;if(k==j.OCCUR){e++;}j.mSR();for(var d=j.N.J-e;d>-1;--d){if(j.sIO(d)){j.hC();
j.go(j.S[d].J);return;}}j.warn("Søk bakover: tekst ikke funnet.",false,j.OCCUR);j.OCCUR=g;
return;}else{if(a=="R"){for(var d=j.N.J-1;d>-1;--d){k=j.rT(j.S[d].H.innerHTML);if(j.sIO(d)){j.hC();
j.go(j.S[d].J);return;}}j.warn("Søk bakover: tekst ikke funnet.",false,j.OCCUR);return;
}else{if(a=="o"){if(""==k){return false;}if(j.SHO){j.rSH();}var g=j.OCCUR;j.OCCUR=k;
j.mSR();var b=new Array();for(var d=0;d<j.S.length;++d){if(j.sIO(d)){b.push(d);}}if(0==b.length){j.warn("Occur: tekst ikke funnet.",false,j.OCCUR);
j.OCCUR=g;return;}j.hC();if(j.PLAIN_VIEW!=j.VIEW){j.pV();}j.R.DRT=true;j.tG();for(var d=0;
d<j.S.length;++d){OrgNode.sE(j.S[d].D);OrgNode.hE(j.S[d].F);}for(var d=(b.length-1);
d>=1;--d){OrgNode.sE(j.S[b[d]].F);}j.ss(b[0]);}else{if(a==j.RC_O){var h=k.charAt(0);
if("s"==h){j.sR(j.RC_NULL,"Org-link to this section:","[["+j.BU+j.dT()+"]["+document.title+", Sec. '"+j.rT(j.N.H.innerHTML)+"']]","C-c to copy, ");
...
window.setTimeout(function(){org_html_manager.CI.select();},100);}else{j.warn(h+": Ingen slik lenketype.");
}}}else{if(a==j.RC_H){var h=k.charAt(0);if("s"==h){j.sR(j.RC_NULL,"HTML-lenke til denne seksjonen:",'<a href="'+j.BU+j.dT()+'">'+document.title+", Sec. '"+j.rT(j.N.H.innerHTML)+"'</a>","C-c to copy, ");
...
if(a.Hg){a.LVM=a.VIEW;if(a.PLAIN_VIEW==a.VIEW){a.iV(true);}a.W.innerHTML='Hvilken som helst knapp eller <a href="javascript:org_html_manager.showHelp();">trykk her</a> for å fortsette.<h2>Tastatursnarveier</h2><table cellpadding="3" rules="groups" frame="hsides" style="caption-side:bottom;margin:20px;border-style:none;" border="0";><caption><small>org-info.js, v. 0.1.7.2</small></caption><tbody><tr><td><code><b>?</b></code></td><td>vis denne hjelpeskjermen</td></tr></tbody><tbody><tr><td><b>Flytte rundt</b></td></tr><tr><td><code><b>n / p</b></code></td><td>gå til neste / forrige delkapittel</td></tr><tr><td><code><b>N / P</b></code></td><td>gå til neste / forrige kapittel</td></tr><tr><td><code><b>t / E</b></code></td><td>gå til første / siste delkapittel</td></tr><tr><td><code><b>g</b></code></td><td>gå til delkapittel...</td></tr><tr><td><code><b>u</b></code></td><td>gå opp et nivå</td></tr><tr><td><code><b>i / C</b></code></td><td>vis innholdsfortegnelse / indeks</td></tr><tr><td><code><b>b / B</b></code></td><td>gå tilbake til siste / neste besøkte delkapittel</td></tr><tr><td><code><b>h / H</b></code></td><td>gå til hovedindeks for dette kapittelet / «hjemme»</td></tr></tbody><tbody><tr><td><b>Visninger</b></td></tr><tr><td><code><b>m / x</b></code></td><td>endre mellom informativ og enkel visning / slides</td></tr><tr><td><code><b>f / F</b></code></td><td>brett sammen gjeldende visning / vis hele dokumentet</td></tr></tbody><tbody><tr><td><b>Søking</b></td></tr><tr><td><code><b>s / r</b></code></td><td>søk framover / bakover...</td></tr><tr><td><code><b>S / R</b></code></td><td>søk igjen framover / bakover...</td></tr><tr><td><code><b>c</b></code></td><td>klarer søkemarkering</td></tr></tbody><tbody><tr><td><b>Ymse</b></td></tr><tr><td><code><b>l / L / U</b></code></td><td>vis HTML-lenke / Org-lenke / vanlig-URL</td></tr><tr><td><code><b>v / V</b></code></td><td>scroll oppgver / nedover</td></tr><tr><td><code><b>W</b></code></td><td>print</td></tr></tbody></table><br />Hvilken som helst knapp eller <a href="javascript:org_html_manager.showHelp();">trykk her</a> for å fortsette.';
...
}}e.Hg=e.Hg?0:1;if(e.Hg){e.LVM=e.VIEW;if(e.PLAIN_VIEW==e.VIEW){e.iV(true);}if(null==e.TAGS_INDEX){e.TAGS_INDEX='Hvilken som helst knapp eller <a href="javascript:org_html_manager.showTagsIndex();">trykk her</a> for å fortsette.<br /><br />Trykk på overskriftene for å utvide innholdet.<h2>Emneknaggindeks:</h2>';
...
}e.TAGS_INDEX+="</ul></div>";}e.TAGS_INDEX+='<br />Hvilken som helst knapp eller <a href="javascript:org_html_manager.showTagsIndex();">trykk her</a> to proceed.';
}e.W.innerHTML=e.TAGS_INDEX;window.scrollTo(0,0);}else{if(e.PLAIN_VIEW==e.LVM){e.pV();
}else{if(e.SLIDE_VIEW==e.LVM){e.sV();}}e.ss(e.N.J);}},showSyntax:function(){var a=this;
a.SYNTAXING=a.SYNTAXING?0:1;if(a.SYNTAXING){a.LVM=a.VIEW;if(a.PLAIN_VIEW==a.VIEW){a.iV(true);
}a.W.innerHTML="TESTSYNTAKS";window.scrollTo(0,0);}else{if(a.PLAIN_VIEW==a.LVM){a.pV();
}else{if(a.SLIDE_VIEW==a.LVM){a.sV();}}a.ss(a.N.J);}},runHooks:function(b,a){if(this.HOOKS.run_hooks&&this.HOOKS[b]){for(var c=0;
